 ...
 * 
 * @param {Array<Object>} resultado_ejecucion - Array of order data objects (required)
 * @param {string} cliente_sellout - Customer name/code
 ...
 */
export async function Create_Email_HTML(resultado_ejecucion, cliente_sellout, asunto) {
    try {
        const result = await Call_CreateEmailHTML(resultado_ejecucion, cliente_sellout, asunto)
 ...
 */
async function Call_CreateEmailHTML(resultado_ejecucion, cliente_sellout, asunto) {
    let httpStatusCode
    let body
    ...
    try {
        ...
        const tableRowsHTML = generateTableRows(resultado_ejecucion)

        // Step 3: Replace placeholder with actual rows
        let completeHTML = emailTemplate.replaceAll('[[ROWS]]', tableRowsHTML)

        completeHTML = completeHTML.replaceAll('[[CLIENTE_SELLOUT]]', cliente_sellout)
        completeHTML = completeHTML.replaceAll('[[RESULTS]]', tableRowsHTML.resultado.join(''))
        completeHTML = completeHTML.replaceAll('[[SUBJECT]]', asunto)
                                    ...
                                    <td
                                        ...
                                        <div style="text-align: center; padding-top: 10px;">
                                            <span
                                                ...
                                                <b style="font-weight: bold;">RESULTADOS PROCESAMIENTO</b>.                                                
                                            </span>
                                        </div>
 ...
 */
function generateTableRows(resultado_ejecucion) {
    const resultado = []

    for (const rowData of resultado_ejecucion) {
        ...
        row += `<td style="padding: 10px 4px; border: 1px solid #e5e7eb; font-size: 12px; text-align: center; color: #374151;">${rowData.result.join('\n')}</td>\n`
        row += '</tr>\n'
        resultado.push(row)
    }
